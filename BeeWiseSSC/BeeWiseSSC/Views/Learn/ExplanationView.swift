        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                infoCard(color: .green) {
                    Text("Detecting Varroa early is the difference between a small, targeted treatment and losing the entire colony. BeeWiseSSC replaces destructive counting methods (sugar shake, alcohol wash) with a non-invasive photo scan.")
                        .font(.body)
                }
                ...
                infoCard(color: .blue) {
                    Text("The Honey Bee Health Coalition recommends monitoring at least once a month during the active season. A colony that crosses the 3% threshold can collapse within 4–6 weeks without intervention. BeeWiseSSC makes frequent monitoring practical.")
                        .font(.body)
                }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                infoCard(color: .brown) {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("BeeWiseSSC's detection pipeline directly implements the approach described in this peer-reviewed paper:")
                            .font(.body)
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                infoCard(color: .indigo) {
                    VStack(alignment: .leading, spacing: 10) {
                        Text("The core of BeeWiseSSC is powered by a custom object detection model built entirely with **Create ML**.")
                            .font(.body)
